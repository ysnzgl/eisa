<script setup>
/**
 * QR Scan — Eczacının kiosk QR kodunu okutarak
 * hastanın anket oturumunu görüntülemesi.
 *
 * Önce tarayıcının BarcodeDetector API'sini dener (kamera ile otomatik okuma),
 * çalışmazsa eczacı kodu manuel olarak girer.
 */
...
async function startCamera() {
  try {
    ...
    // Video elementine stream ata (nextTick sonrası)
    setTimeout(() => {
      if (videoRef.value) {
...
async function scanFrame() {
  ...
  try {
    ...
    // Kare okunamadı, bir sonraki kareyi bekle
  }
}
...
async function lookup() {
  ...
  try {
    // Merkezi API üzerinden sorgula
    const res = await http.get('/api/analytics/sessions/', { params: { qr_code: code } });
    const results = Array.isArray(res.data) ? res.data : res.data.results ?? [];
    if (results.length > 0) {
      session.value = results[0];
    } else {
      notFound.value = true;
    }
  } catch {
    apiError.value = 'Sorgulama başarısız. Lütfen tekrar deneyin.';
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="p-6 max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow p-5 space-y-4">
      ...
      <div class="flex gap-2">
        <input
          v-model="qrInput"
          @keyup.enter="lookup"
          placeholder="Örn: A3F92C1D8E7B"
          class="flex-1 border rounded-lg px-4 py-2 text-sm font-mono tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-500 uppercase"
          :disabled="loading"
        />
        <button
          @click="lookup"
    ...
    <!-- Hata -->
    <div v-if="apiError" class="bg-red-50 text-red-700 border border-red-200 rounded-lg p-3 text-sm">
      {{ apiError }}
    </div>
    ...
    <div v-if="notFound" class="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-xl p-5 text-center">
      ...
      <p class="text-sm mt-1">
        <span class="font-mono font-bold">{{ qrInput.trim().toUpperCase() }}</span> koduna ait kayıt yok.
      </p>
      <button @click="reset" class="mt-3 text-sm text-blue-600 hover:underline">Yeniden dene</button>
    </div>

    <!-- Oturum Detayı -->
    <div v-if="session" class="bg-white rounded-xl shadow divide-y divide-gray-100">
      <!-- Başlık -->
      ...
      <div class="px-5 py-4 grid grid-cols-2 gap-4">
        ...
        <div class="col-span-2">
          ...
          <p class="font-semibold text-gray-800">{{ session.category?.name ?? session.category_id }}</p>
        </div>
      </div>
      ...
      <!-- Anket Cevapları (Hassas değilse göster) -->
      <div v-if="!session.is_sensitive_flow && session.answers_payload && Object.keys(session.answers_payload).length" class="px-5 py-4">
      ...
      <div class="px-5 py-3 bg-gray-50 flex items-center justify-between">
        <span class="text-xs text-gray-500">
          Kiosk: <span class="font-medium text-gray-700">{{ session.kiosk?.pharmacy?.name ?? '—' }}</span>
        </span>
