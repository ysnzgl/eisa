 ...
 * Modül 3: Reklam oluturma, hedefleme, sekmeli liste
 */
...
async function saveForm() {
  if (!form.value.starts_at || !form.value.ends_at) {
    drawerError.value = 'Başlangıç ve biti tarihi zorunludur.';
    return;
  }
  if (new Date(form.value.starts_at) >= new Date(form.value.ends_at)) {
    drawerError.value = 'Biti tarihi başlangıçtan sonra olmalıdır.';
    return;
  }
  ...
  try {
    ...
    await loadCampaigns();
    closeDrawer();
  } catch { drawerError.value = 'Kayt baarsz. Lütfen tekrar deneyin.'; }
...
const TAB_CONFIG = [
  ...
  { key: 'upcoming', label: 'Yaklaan',  dot: 'bg-amber-400'   },
  { key: 'ended',    label: 'Tamamland',dot: 'bg-zinc-500'    },
];
</script>
...
<template>
  <div class="cm-root min-h-screen">
    ...
    <div class="cm-topbar px-8 py-6 flex items-start justify-between border-b border-slate-800/60">
      <div>
        <p class="eyebrow mb-1">Kiosk Yayn Sistemi</p>
        <h1 class="page-title">Reklam Yöneticisi</h1>
        <p class="text-sm text-slate-400 mt-1 font-light tracking-wide">Idle-screen reklam içeriklerini planlayn, hedefleyin ve izleyin.</p>
      </div>
    ...
    <div class="px-8 pt-6 pb-10">
      ...
      >
        ...
        <p class="text-sm font-medium">Bu sekme için REKLAM bulunamad.</p>
        <button v-if="activeTab !== 'ended'" @click="openCreate" class="mt-3 text-orange-500 text-sm hover:text-orange-600 underline underline-offset-2">lk REKLAMy olutur </button>
      </div>
      ...
      <div v-else class="space-y-4">
        ...
        >
          ...
          <div class="flex-1 min-w-0 py-4 pr-4">
            ...
            <!-- Biten REKLAMlar -->
            <template v-else>
              <span class="text-xs text-slate-600 font-mono">{{ fmtDate(c.ends_at) }} tarihinde tamamland</span>
            </template>
          </div>
        </div>
      </div>
      ...
      <aside v-if="drawerOpen" class="cm-drawer fixed right-0 top-0 h-full z-50 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <template v-if="drawerStep === 1">
            <div>
              <label class="field-label">REKLAM Ad <span class="req">*</span></label>
              <input v-model="form.name" type="text" placeholder="Bahar Vitamin REKLAMs" class="field-input"/>
            </div>
            <div>
              <label class="field-label">Müşteri / Firma Ad <span class="req">*</span></label>
              <input v-model="form.client" type="text" placeholder="Eczacba Salk A.." class="field-input"/>
            </div>
            <div>
            ...
            <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
              <div>
                <p class="text-sm font-semibold text-slate-200">Aktif Olarak Yaynla</p>
                <p class="text-xs text-slate-500">Kapalysa taslak olarak kaydedilir</p>
              </div>
              <button
          ...
          <template v-if="drawerStep === 2">
            ...
            >
              ...
              <template v-if="uploading">
                <svg class="animate-spin w-8 h-8 text-orange-500 mb-3" fill="none" viewBox="0 0 24 24">
              ...
              <template v-else-if="form.media_url">
                ...
                <p class="text-xs text-gray-400 mt-0.5">Deitirmek için tıkla veya sürükle</p>
              </template>
              ...
              <template v-else>
                ...
                <p class="text-sm font-semibold text-gray-600">Dosya seç veya sürükle brak</p>
          ...
          <template v-if="drawerStep === 3">
            ...
            <!-- Tarih Aral -->
            <div>
              <label class="field-label">Yayn Tarihi Aral <span class="req">*</span></label>
              <div class="grid grid-cols-2 gap-3 mt-2">
                ...
                <div>
                  <p class="text-[11px] text-slate-500 mb-1 font-medium">Biti</p>
          ...
          <!-- Gün İçi Saatler -->
            <div>
              <label class="field-label">Günlük Yayn Saatleri</label>
              <p class="text-xs text-gray-500 mb-2">Kiosk'larda hangi saatler aras gösterilecek</p>
              <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                  <p class="text-[11px] text-slate-500 mb-1 font-medium">Biti Saati</p>
            ...
            <div>
              ...
              <div class="flex items-center justify-between mb-2">
                <p class="text-xs text-slate-400">
                  <span v-if="form.target_pharmacy_ids.length === 0" class="text-slate-500">Seçim yok  Tüm eczanelerde yayınlanır</span>
              ...
              <div class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                <div v-if="allPharmacies.length === 0" class="text-xs text-slate-500 py-4 text-center">Eczaneler yükleniyor…</div>
                <button
        ...
        <div class="drawer-footer flex-shrink-0 flex items-center gap-3">
          ...
          <button v-else @click="saveForm" :disabled="drawerSaving" class="flex-1 save-btn">
            ...
            {{ drawerSaving ? 'Kaydediliyor…' : (drawerMode === 'create' ? 'Reklam Olutur' : 'Güncelle') }}
          </button>
        </div>
      </aside>
    </Transition>
        ...
        <Transition name="modal" appear>
          <div v-if="deleteTarget" class="delete-modal w-full max-w-sm">
            <div class="p-6 text-center">
              ...
              <h3 class="text-sm font-bold text-gray-800 mb-1">REKLAMy Sil</h3>
              <p class="text-xs text-gray-500 leading-relaxed">
                <span class="text-gray-700 font-semibold">"{{ deleteTarget?.name }}"</span> REKLAMs kalc olarak silinecek.
              </p>
            </div>
