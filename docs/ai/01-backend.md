...
- `Playlist`: Kiosk için üretilmiş playlist (kiosk FK, target_date, target_hour, version, locked, total_duration_seconds, items JSON, metadata JSON)
- `PlaylistItem`: Playlist öğeleri (playlist FK, asset_type [creative/house_ad], asset_id, duration_seconds, playback_order)
- `PlaylistTemplate`: Playlist şablonları (name, target_hour, locked, items JSON)
- `HouseAd`: Filler reklam (name, media_url, duration_seconds, priority, active, checksum)
- `PlayLog`: Reklam gösterim logu (kiosk FK, playlist FK, creative/house_ad FK, play_started, play_ended, completed, failure_reason)
